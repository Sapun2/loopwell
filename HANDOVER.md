...
| Gradle build fails and the whole error is a version number, e.g. `What went wrong: 26.0.1` | Your JDK is too new for Gradle. `brew install --cask temurin@17` **first**, then `flutter config --jdk-dir "$(/usr/libexec/java_home -v 17)"`. Run in that order: `java_home -v 17` returns the newest JDK when 17 is absent, so running it first points Flutter at the wrong one |
| `Could not locate aapt` after `✓ Built ...app-debug.apk` | The app built; only Flutter's install step failed. Install it directly: `adb install -r build/app/outputs/flutter-apk/app-debug.apk` then `adb shell monkey -p com.example.loopwell -c android.intent.category.LAUNCHER 1` |
